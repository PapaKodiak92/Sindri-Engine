...
internal sealed class VictoryScene : Scene2D
{
    ...
    protected override void OnEnter(SceneContext context)
    {
        ...
        var title = CreateEntity("Victory Title");
        ...
        title.AddComponent(new TextRenderer2D("VICTORY", ColorRGBA.SindriGold)
        {
            RenderSpace = RenderSpace.Screen,
            RenderLayer = 10_000
        });

        var instructions = CreateEntity("Victory Instructions");
        ...
        Console.WriteLine("Victory. Press Enter to restart or Escape to exit.");
    }
